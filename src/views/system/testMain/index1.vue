<template>
  <div class="app-container">
    ...
    <el-table ref="subTable" v-loading="loading" :data="testMainList" @selection-change="handleSelectionChange"
              :row-key="getRowKeys"
      ...
      <el-table-column type="expand" align="center">
        ...
        <template #default="scope">
          <el-table size="small" :border="true" v-loading="subLoading" :data="scope.row.testSubList">
            <el-table-column label="测试子表主键" align="center" prop="id" show-overflow-tooltip/>
            <el-table-column label="子表名称" align="center" prop="testSubName"/>
            <el-table-column
                label="操作"
                align="center"
                header-align="center"
            ...
            >
              <template #default="scope">
                <el-button
                    ...
                    type="text"
                    icon="Edit"
                    @click="handleUpdateTestSub(scope.row)"
                ...
                <el-button
                    ...
                    type="text"
                    icon="Delete"
                    @click="handleDeleteTestSub(scope.row)"
    ...
    <el-dialog :title="testSubTitle" v-model="openTestSub" width="500px" append-to-body>
      <el-form ref="testSubRef" :model="testSubForm" :rules="testSubRules" label-width="80px">
        <el-form-item label="测试名称" prop="testName">
          <el-input v-model="testSubForm.testSubName" placeholder="请输入测试名称"/>
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
...
import {
  ...
  getTestMainList,
  updateTestMain
} from "@/api/system/testMain";
import {
  addTestSubRequest,
  deleteTestSubByIdsRequest,
  getTestSubDetailsRequest,
  getTestSubListRequest,
  updateTestSubRequest
} from "@/api/system/testSub";
...
const {queryParams, testSubForm, getRowKeys, expandRowKeys, form, rules, testSubRules} = toRefs(data);

/** 查询测试列表 */
async function getList() {
  loading.value = true;
  let response = await getTestMainList(queryParams.value)
  testMainList.value = response.data.list;
  let subTable = proxy.$refs["subTable"];
  if (subTable) {
    for (let item of expandRowKeys.value) {
      subTable.toggleRowExpansion({id: item}, false)
    }
  }
...
async function handleUpdate(row) {
  ...
  let response = await getTestMainDetails(id);
  ...
  proxy.$refs["testMainRef"].validate(async valid => {
    if (valid) {
      ...
      if (form.value.id != null) {
        await updateTestMain(form.value);
        proxy.$modal.msgSuccess("修改成功");
        open.value = false;
        await getList();
      } else {
        await addTestMain(form.value);
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除测试编号为"' + idList + '"的数据项？').then(async function () {
    await deleteTestMainByIds(idList);
    await getList();
  ...
  if (code === 200) {
    testMainList.value.forEach((item, index) => {
      if (item.id === testMainId) {
        testMainList.value[index1].testSubList = data; // 更新[] => 当前展开行请求到的列表数据
      }
    })
...
function handleDeleteTestSub(row) {
  ...
  proxy.$modal.confirm('是否确认删除岗位班次编号为"' + id + '"的数据项?', "警告", {
...
// 编辑子表数据
async function handleUpdateTestSub(row) {
  if (!row) return;
  const {id} = row;
  ...
  proxy.$refs["testSubRef"].validate(async valid => {
    if (valid) {
      if (testSubForm.value.id != null) {
        await updateTestSubRequest(testSubForm.value);
        proxy.$modal.msgSuccess("修改成功");
